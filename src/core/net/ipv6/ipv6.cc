...
void IPv6::finish() {
    if (DEBUG)
        std::cout << "Packet remaining: " << ipPacketQueue.size() << " @ "
                << getParentModule()->getFullName()
//      << " _ " << check_and_cast<RDCdriver*>(getModuleByPath("^.rdc"))->phase
                << endl;

    // Clear queue !!!
    for (std::list<IpPacketInterface*>::iterator it =
            this->ipPacketQueue.begin(); it != this->ipPacketQueue.end(); it++)
        cancelAndDelete(*it);
...
void IPv6::processLowerLayerMessage(cPacket* packet) {
    ...
    case DATA: /* Data */
    {
        ...
        if (getModuleByPath("^.^")->par("usingHDC").boolValue()) {
            ...
            // received MAC !!!
            FrameDataCompressed* frame = check_and_cast<FrameDataCompressed*>(
                    packet);
            IpPacketCompressed* ipPacket = check_and_cast<IpPacketCompressed*>(
                    frame->decapsulate());

            // check right final destination
            if (frame->getFinalDestinationMacAddress() == 0
                    || frame->getFinalDestinationMacAddress()
                            == this->getModuleByPath("^.mac")->getId()) {
                // process
                switch (ipPacket->getNextHeader()) {
                case NEXT_HEADER_UDP: /* UDP */
                {
                    (check_and_cast<Statistic*>(
                            getModuleByPath("^.^.statistic")))->registerStatistic(
                    NET_RECV); // statistics
                    ...
                    if (simulation.getModuleByPath("WSN") != NULL) {
                        ...
                        if (getModuleByPath("^.^")->par("isPollingCount").boolValue()) {
                            check_and_cast<Count*>(
                                    getParentModule()->getSubmodule("count"))->receive(
                                    check_and_cast<IpPacketCompressed*>(
                                            ipPacket)->getBitLength());

                            check_and_cast<Statistic*>(
                                    getModuleByPath("^.^.statistic"))->registerStatisticDelay(
                            DELAY_NET_LAYER,
                                    simTime().dbl()
                                            - check_and_cast<IpPacketCompressed*>(
                                                    ipPacket)->getTime()); // statistics
                        }
                    }
                ...
                case NEXT_HEADER_ICMP: /* ICMP */
                {
                    ...
                    IcmpPacket* icmpPacket = check_and_cast<IcmpPacket*>(
                ...
                if (frame->getHopLeft() == 0) {
                    ...
                    if (getModuleByPath("^.^")->par("usingFLR").boolValue()) {
                        for (std::list<RPL_neighbor*>::iterator it =
                                this->rpl->rplDag->parentList.begin();
                                it != this->rpl->rplDag->parentList.end();
                                it++) {
                            if ((*it)->neighborID
                                    == simulation.getModule(
                                            frame->getDestinationMacAddress())->getModuleByPath(
                                            "^.net")->getId()) {
                        ...
                        for (std::list<RPL_neighbor*>::iterator it =
                                this->rpl->rplDag->siblingList.begin();
                                it != this->rpl->rplDag->siblingList.end();
                                it++) {
                            if ((*it)->neighborID
                                    == simulation.getModule(
                                            frame->getDestinationMacAddress())->getModuleByPath(
            ...
            case NEXT_HEADER_UDP: /* UDP */
            {
                if (simulation.getModuleByPath("WSN") != NULL) {
                    ...
                    if (getModuleByPath("^.^")->par("isPollingCount").boolValue()) {
                        ...
                        DELAY_NET_LAYER,
                                simTime().dbl()
                                        - check_and_cast<IpPacketStandard*>(
                                                packet)->getTime()); // statistics
                    }
                }

                if (check_and_cast<IpPacketStandard*>(packet)->getDestinationIpAddress()
                        != 0
                        && check_and_cast<IpPacketStandard*>(packet)->getDestinationIpAddress()
                                != this->getId()) {
                    // hop limit !!!
                    if (check_and_cast<IpPacketStandard*>(packet)->getHopLimit()
                            == 1) {
                        delete packet;
                    } else {
                        // decrease hop limit
                        check_and_cast<IpPacketStandard*>(packet)->setHopLimit(
                                check_and_cast<IpPacketStandard*>(packet)->getHopLimit()
                                        - 1);
...
// Energy calculated by counting bits
    ...
    case RESULT: /* Result */
    {
        ...
        case MAC_FINISH_PHASE: /* MAC layer finish a transmitting phase */
        {
            ...
            if (getModuleByPath("^.^")->par("usingHDC").boolValue()) {
                // compress using HC01
                if (check_and_cast<IpPacketCompressed*>(bufferNET)->getNextHeader()
                        == NEXT_HEADER_UDP) {
                    if (simulation.getModuleByPath("WSN") != NULL) {
                        (check_and_cast<Statistic*>(
                                getModuleByPath("^.^.statistic")))->registerStatistic(
                ...
                } else if (check_and_cast<IpPacketCompressed*>(bufferNET)->getNextHeader()
                        == NEXT_HEADER_ICMP) {
                    if (check_and_cast<IcmpPacket*>(
                            bufferNET->getEncapsulatedPacket())->getType()
                            == ICMP_RPL) {
                        if (check_and_cast<IcmpPacket*>(
                                bufferNET->getEncapsulatedPacket())->getCode()
                                == RPL_DIO_CODE) {
                            if (simulation.getModuleByPath("WSN") != NULL) {
                                (check_and_cast<Statistic*>(
                            ...
                            this->rpl->hasSentDIO();
                        } else if (check_and_cast<IcmpPacket*>(
                                bufferNET->getEncapsulatedPacket())->getCode()
                                == RPL_DIS_CODE) {
                            if (simulation.getModuleByPath("WSN") != NULL) {
                                (check_and_cast<Statistic*>(
                ...
                if (check_and_cast<IpPacketStandard*>(bufferNET)->getNextHeader()
                        == NEXT_HEADER_UDP) {
                    if (simulation.getModuleByPath("WSN") != NULL) {
                        (check_and_cast<Statistic*>(
                                getModuleByPath("^.^.statistic")))->registerStatistic(
                ...
                } else if (check_and_cast<IpPacketStandard*>(bufferNET)->getNextHeader()
                        == NEXT_HEADER_ICMP) {
                    if (check_and_cast<IcmpPacket*>(
                            bufferNET->getEncapsulatedPacket())->getType()
                            == ICMP_RPL) {
...
void IPv6::preparePacketToBeSent() {
    ...
    if (this->defaultRoute != 0) {
        bufferNET = check_and_cast<IpPacketInterface*>(
                this->ipPacketQueue.front());
    } else {
        // search for DIS
        for (std::list<IpPacketInterface*>::iterator it =
                this->ipPacketQueue.begin(); it != this->ipPacketQueue.end();
                it++) {
            if (getModuleByPath("^.^")->par("usingHDC").boolValue()) {
                // compress using HC01
                if (check_and_cast<IpPacketCompressed*>(*it)->getNextHeader()
                        == NEXT_HEADER_ICMP) {
                    if (check_and_cast<IcmpPacket*>(
                            (*it)->getEncapsulatedPacket())->getType()
                            == ICMP_RPL) {
                        if (check_and_cast<IcmpPacket*>(
                                (*it)->getEncapsulatedPacket())->getCode()
                                == RPL_DIS_CODE) {
                            this->bufferNET = *it;
                            break;
                        }
                ...
                if (check_and_cast<IpPacketStandard*>(*it)->getNextHeader()
                        == NEXT_HEADER_ICMP) {
                    if (check_and_cast<IcmpPacket*>(
                            (*it)->getEncapsulatedPacket())->getType()
                            == ICMP_RPL) {
...
//      if (check_and_cast<IpPacketCompressed*>(bufferNET)->getNextHeader() == NEXT_HEADER_UDP)
                (check_and_cast<Statistic*>(getModuleByPath("^.^.statistic")))->registerStatistic(
                NET_SEND);
            }
        } else {
            // statistics
...
void IPv6::multicast(IcmpPacket *icmpPacket) {
    ...
    if (getModuleByPath("^.^")->par("usingHDC").boolValue()) {
        ...
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setNextHeader(
                NEXT_HEADER_ICMP);
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setHopLimit(
        HOP_LIMIT_COMPRESSED_64);
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setSourceIpAddress(3); // Ox11: PC, IC
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setDestinationIpAddress(
                3); // 0x11: PC, IC

        // meta-data as arguments for mac functions
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setMetaHopLimit(64);
        (check_and_cast<IpPacketCompressed*>(ipPacket))->setMetaSourceIpAddress(
                this->getId());
        ...
        (check_and_cast<IpPacketStandard*>(ipPacket))->setNextHeader(
                NEXT_HEADER_ICMP);
        (check_and_cast<IpPacketStandard*>(ipPacket))->setHopLimit(
        DEFAULT_HOP_LIMIT);
        (check_and_cast<IpPacketStandard*>(ipPacket))->setSourceIpAddress(
                this->getId());
