...
void contikiMAC::processLowerLayerMessage(cPacket* packet) {
...
void contikiMAC::selfProcess(cPacket* packet) {
    switch (packet->getKind()) {
...
void contikiMAC::quitMACtransmissonPhase() {
    // release lock
    phase = FREE_PHASE;
...
void contikiMAC::enterRDCtransmissionPhase() {
...
void contikiMAC::quitRDCtransmissionPhase(int result) {
    // stop transmission phase
    if (DEBUG)
